...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan) {
    ...
    {
        if (hcan == &hcan1 && Rxmessage.StdId == 0x612)
        {
            uint16_t m_tmp;     
            m_tmp = (uint16_t)rx_data[0] << 8 | rx_data[1];	
            pm01->p_out = m_tmp;

            m_tmp = (uint16_t)rx_data[2] << 8 | rx_data[3];	
            pm01->v_out = m_tmp;	

            m_tmp = (uint16_t)rx_data[4] << 8 | rx_data[5];	
            pm01->i_out = m_tmp;
        }
