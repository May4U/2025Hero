...
////    /* 使能串口DMA接收 */
////		//HAL_UART_Receive_DMA(&huart1, Usart1_Rx, USART1_RX_LEN); // 开启串口DMA接收（记得开启循环模式）
////    if (fifo_init(pfifo_visual, Usart1_Rx_Buffer, USART1_RX_LEN) == -1)
////    {
////        Error_Handler(); // 必须 2 的幂次方
////    }

////    /* 使能串口DMA发送 */  //成功
////   __HAL_DMA_DISABLE(huart1.hdmatx);  // 开启串口DMA发送通道 （不用开启循环模式）
