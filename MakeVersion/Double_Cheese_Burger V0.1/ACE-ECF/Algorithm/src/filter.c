...
{
    for (int i = 0; i < 300; i++)   filter->fifo[i] = 0;
	filter->count_num = 0;
	filter->sum = 0;
    filter->filter_out = 0;
	return 0;
...
float Recursive_ave_filter(Recursive_ave_filter_type_t *filter,float input,int num)
{
	...
	if(filter->count_num < num)//终止条件为count_num = num;
	{
		filter->count_num ++;
	}else
    {
        filter->count_num = num;
    }
	
	filter->sum = 0;
	for(i = 0; i < filter->count_num; i++)//防止取样数量 < max时 滤波错误
	{
		filter->sum += filter->fifo[i]; 
	}
	filter->filter_out = filter->sum/filter->count_num;
	return filter->filter_out;
}
